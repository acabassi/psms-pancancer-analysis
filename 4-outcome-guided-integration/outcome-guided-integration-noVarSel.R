...
weights <-
  as.vector(
    readMat(
      paste0("../outcome-guided-integration-output/outcomeguided_integration",
...
n_datasets <- 4
datasetNames <- c("CN", "Methylation", "miRNA", "RPPA")
N <- dim(psm_cn)[1]
weighted_kernel <- array(NA, c(N, N))
dimnames(weighted_kernel) <- list(rownames(psm_cn), rownames(psm_cn))

weighted_kernel <- weights[1]*psm_cn + weights[2]*psm_methylation +
  weights[3]*psm_mirna + weights[4]*psm_rppa
...
for(i in 2:50){
    ...
    kkm <- kkmeans(weighted_kernel, parameters, seed = 4)
...
save(
  weighted_kernel,
    maxSil,
    file = paste0(
      "../outcome-guided-integration-output/outcomeguided_output_all",
      var_sel,".RData")
)
...
our_cl_palette <-  c(
  "1" = "#6CACE4" , "2" = "#0072ce", "3" =  "#E89CAE", "4" = "#8A1538",
  "5" = "#E89CAE", "6" = "#af95a6", "7" = "#8A1538", "8" = "#B7BF10",
  "9" = "#B7BF10", "10" ="#0072ce", "11" = "#0072ce", "12" = "#af95a6",
  "13" = "#F1BE48", "14" = "#E47113", "15" = "#D50032", "16" = "#0072ce",
  "17" = "#B7BF10", "18" = "#6CACE4", "19" = "#B7BF10",
  "20" = "#6CACE4", "21" = "#6CACE4", "22" = "#6CACE4", "23" = "#B7BF10",
  "24" = "#B7BF10", "25" = "#E47113", "26" = "#af95a6",  "27" = "#B7BF10"
)
...
names(tissue_palette) <- c("BLCA", "BRCA", "COAD", "HNSC", "KIRC",
...
pdf(
  paste0("../figures/outcomeguided_weightedKernel_",
         bestK, "clusters", var_sel, ".pdf"),
  height = 6,
  width = 10
)
draw(
...
pdf(paste0("../figures/outcomeguided_pancan_silhouette", var_sel,".pdf"),
    height = 3.75,
    width = 6
    )
plot(2:maxK,
...
######################### Plot data with final clusters ######################## 

CN_data <- as.matrix(read.csv("../data/CN.csv", row.names = 1))
col_fun <-
  colorRamp2(c(min(CN_data), 0, max(CN_data)),
             c("#D55E00", "white", "#0072B2"))

HdataMatrix <-
  Heatmap(
    CN_data[sortedClusters$ix, ],
    cluster_columns = TRUE,
    show_column_dend = FALSE,
    cluster_rows = FALSE,
    show_column_names = FALSE,
    show_row_names = FALSE,
    name = "Copy number data",
    col = col_fun,
    right_annotation = Hclusters,
    show_heatmap_legend = FALSE,
    width = unit(13, "cm"))

pdf(paste0("../figures/outcomeguided_cn", var_sel, ".pdf"))
HdataMatrix
dev.off()

methylation_data <-
  as.matrix(read.csv("../data/methylation.csv", row.names = 1))
col_fun <-
  colorRamp2(c(0, max(methylation_data)),
             c("white", "#0072B2"))
HdataMatrix <-
  Heatmap(
    methylation_data[sortedClusters$ix, ],
    cluster_columns = TRUE,
    show_column_dend = FALSE,
    cluster_rows = FALSE,
    show_column_names = FALSE,
...
pdf(paste0("../figures/outcomeguided_methylation", var_sel, ".pdf"))
HdataMatrix
dev.off()

 miRNA_data <-
  as.matrix(read.csv("../data/miRNA.csv", row.names = 1))
col_fun <-
  colorRamp2(c(min(miRNA_data), 0, max(miRNA_data)),
             c("#D55E00", "white", "#0072B2"))
HdataMatrix <-
  Heatmap(
    miRNA_data[sortedClusters$ix, ],
    cluster_columns = TRUE,
    show_column_dend = FALSE,
    cluster_rows = FALSE,
    show_column_names = FALSE,
...
pdf(paste0("../figures/outcomeguided_miRNA", var_sel, ".pdf"))
HdataMatrix
dev.off()

RPPA_data <-
  as.matrix(read.csv("../data/RPPA.csv", row.names = 1))
col_fun <-
  colorRamp2(c(min(RPPA_data), 0, max(RPPA_data)),
             c("#D55E00", "white", "#0072B2"))
HdataMatrix <-
  Heatmap(
    RPPA_data[sortedClusters$ix, ],
    cluster_columns = TRUE,
    show_column_dend = FALSE,
    cluster_rows = FALSE,
    show_column_names = FALSE,
...
pdf(paste0("../figures/outcomeguided_RPPA", var_sel, ".pdf"))
HdataMatrix
dev.off()
